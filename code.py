...
db1 = con.connect(host='localhost', user='root', password='1234')
cursor = db1.cursor()
cursor.execute("Create Database library")

# --------------DATABASE "LIBRARY CREATED"

db1 = con.connect(host='localhost', user='root', password='1234', database='Library')
cursor = db1.cursor()

# ----------CONNECT WITH THE DATABASE LIBRARY

str = "Create Table Books(B_Id Varchar(4) , B_Name Varchar(30) , B_Author Varchar(30) , B_Category Varchar (30) , B_Quantity Int, B_Price Int)"
cursor.execute(str)
...
def A():
    ...
    if (id == "" or name == "" or author == "" or category == "" or quantity == "" or price == ""):
        ...
        sql = "Insert Into Books Values ('{}','{}','{}','{}',{},{}) ".format(id, name, author, category, quantity,
                                                                             price)
        cursor.execute(sql)
        db1.commit()
...
def AddBooks():
    ...
    ex = Button(root, text="Exit OUT", width=14, command=lambda: [root.destroy(), panel.deiconify()], bg="red",
...
def AvailBooks():
    ...
    ex = Button(root, text="CLOSE", width=14, command=lambda: [root.destroy(), panel.deiconify()], bg="red", fg="white",
                borderwidth=3)
    ...
    if (crs_id == ""):
        ...
        if course in ac:
            ...
            if response == "yes":
                ...
                Label(win, text="Course_Id :", bg="azure").place(x=100, y=60)
...
def ReturnBooks():
    ...
    id = Label(root, text='Enter Course_Id:', bg="azure")
    id.place(x=100, y=60)
...
def AdminPanel():
    ...
    Button(panel, text="ADD NEW BOOK", command=AddBooks, width=16, borderwidth=8, fg="white", bg="orange").place(x=70,
                                                                                                                 y=130, )
    Button(panel, text="VIEW ALL BOOK", command=AvailBooks, width=16, borderwidth=8, fg="white", bg="orangered").place(
        x=250, y=130)
    Button(panel, text="DELETE A BOOK", command=DeleteBooks, width=16, borderwidth=8, fg="white", bg="orange").place(
        x=430, y=130)
    Button(panel, text="ISSUE A BOOK", command=IssueBooks, width=16, borderwidth=8, fg="white", bg="blue").place(x=70,
                                                                                                                 y=200)
    Button(panel, text="RETURN A BOOK", command=ReturnBooks, width=16, borderwidth=8, fg="white", bg="green").place(
        x=250, y=200)
    Button(panel, text="ISSUE RECORDS", command=IssRecord, width=16, borderwidth=8, fg="white", bg="blue").place(x=430,
                                                                                                                 y=200)
    Button(panel, text="RETURN RECORDS", command=RetRecord, width=16, borderwidth=8, fg="white", bg="green").place(
        x=250, y=270)
...
def UserPanel():
    ...
    Label(panel, text="EMPLOYEE PANEL", font="ComicSansms 12 bold", pady=5, bg="azure").pack(side=TOP)
    Label(panel, text="Select From The Operations Below", font="SansSerif 12 ", pady=20, bg="azure").pack()

    Button(panel, text="VIEW ALL BOOK", command=AvailBooks, width=16, borderwidth=8, fg="white", bg="green").place(x=70,
                                                                                                                   y=130, )
    Button(panel, text="ISSUE A BOOK", command=IssueBooks, width=16, borderwidth=8, fg="white", bg="blue").place(x=250,
                                                                                                                 y=130)
    Button(panel, text="RETURN A BOOK", command=ReturnBooks, width=16, borderwidth=8, fg="white", bg="orange").place(
        x=430, y=130)
...
def Main():
    ...
    display.title("SKYLINE LIBRARY MANAGEMENT")

    time_string = tm.strftime('%H:%M %p')  # time format
    today = date.today()
    ...
    Label(display, text="Copyright 2022. All Rights Reserved To The Creator", bg="azure", font="ComicSansms 8",
          pady=5).pack(side=BOTTOM)
    Label(display, text=d1, font="ComicSansms 10 bold", pady=5, bg="azure").pack(side=BOTTOM)
    Label(display, text=time_string, font="ComicSansms 10 bold", pady=5, bg="azure").pack(side=BOTTOM)
    Label(display, text="Login Time:", font="ComicSansms 10 bold", pady=5, bg="azure").pack(side=BOTTOM)

    Label(display, text="SKYLINE", font="ComicSansms 16 bold", pady=5, bg="azure").pack(side=TOP)
    Label(display, text="MY LIBRARY MY LIFE LINE", font="SansSerif 14 italic", pady=10, bg="azure").pack(side=TOP)
    Label(display, text="Please Select Your User Profile !", font="ComicSansms 12", pady=60, bg="azure").pack()

    user = Button(display, text="LOGIN AS AN EMPLOYEE", width=20, font='sans 10 bold', command=UserPanel, fg="black",
                  bg="chartreuse2", borderwidth=5)
    user.place(x=290, y=190)

    admin = Button(display, text="LOGIN AS AN ADMIN", width=20, font='sans 10 bold', command=Admin, fg="black",
                   bg="deepskyblue2", borderwidth=5)
    admin.place(x=290, y=250)
